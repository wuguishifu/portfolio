        ...
        <div>
            ...
            <div className="py-20 w-full flex flex-col items-center">
                ...
                <div className="gap-8 flex flex-row items-center w-4/5 max-w-screen-xl pt-10">
                    <Masonry gutter="16px">
                        {Object.values(blogs).map((blog, index) => <BlogRow blog={blog} key={index} style={{ animationDelay: `${(index / 3 + 2) * 200}ms` }} />)}
                    </Masonry>
                </div>
            </div>
        </div>
...
function BlogRow({ blog, style }: { blog: any, style?: React.CSSProperties }) {
    return (
        <Link className={`flex flex-col p-4 gap-4 rounded-xl bg-lightGrey items-center animate-fade-in fill-mode-both w-full`} to={`/blog/${blog.slug}`} style={style}>
            {blog.image && <img
